...
func (s *Service) UpdateDSARStatus(ctx context.Context, exportID, status string) error {
	_, err := s.store.DB.Exec(ctx, "UPDATE dsar_exports SET status = $1 WHERE id = $2", status, exportID)
	return err
}
...
func (s *Service) ListAccessLogs(ctx context.Context, tenantID string, limit, offset int) ([]AccessLog, int, error) {
	var total int
	if err := s.store.DB.QueryRow(ctx, "SELECT COUNT(1) FROM access_logs WHERE tenant_id = $1", tenantID).Scan(&total); err != nil {
		total = 0
	}
