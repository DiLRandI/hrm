...
func (s *Service) ManagerIDByEmployeeID(ctx context.Context, tenantID, employeeID string) (string, error) {
	...
	if err := s.store.DB.QueryRow(ctx, "SELECT manager_id FROM employees WHERE tenant_id = $1 AND id = $2", tenantID, employeeID).Scan(&managerID); err != nil {
		return "", err
	}
...
func (s *Service) UpdateReviewTaskStatus(ctx context.Context, tenantID, taskID, status string) error {
	_, err := s.store.DB.Exec(ctx, "UPDATE review_tasks SET status = $1 WHERE tenant_id = $2 AND id = $3", status, tenantID, taskID)
	return err
}
...
func (s *Service) PerformanceSummary(ctx context.Context, tenantID, managerID string) (PerformanceSummary, error) {
	...
	if err := s.store.DB.QueryRow(ctx, goalQuery, goalArgs...).Scan(&summary.GoalsTotal, &summary.GoalsCompleted); err != nil {
		return summary, err
	}
	...
	if err := s.store.DB.QueryRow(ctx, taskQuery, taskArgs...).Scan(&summary.ReviewTasksTotal, &summary.ReviewTasksCompleted); err != nil {
		return summary, err
	}
	...
	rows, err := s.store.DB.Query(ctx, query, responseArgs...)
	var ratings []float64
	if err == nil {
		defer rows.Close()
		for rows.Next() {
	...
	return buildPerformanceSummary(summary.GoalsTotal, summary.GoalsCompleted, summary.ReviewTasksTotal, summary.ReviewTasksCompleted, ratings), nil
}
